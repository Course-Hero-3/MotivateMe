...
export default function TodoForm({formType, 
  ...
  const handleOnCreateFormChange = (event) => {
    ...
    if (createError === "Name field is mandatory" && 
        event.target.name === "name" && event.target.value.length > 0) {
          setCreateError(null)
    }
    if (createError === "Category field is mandatory" && 
        event.target.name === "category" && event.target.value.length > 0) {
          setCreateError(null)
    }
    if (createError === "Due Date field is mandatory" && 
        event.target.name === "dueDate" && event.target.value.length > 0) {
          setCreateError(null)
    }
    if (createError === "Due Time field is mandatory" && 
  ...
  return (
    <div className='todo-form'>
        ...
        {formType === "update"?<TodoUpdate  updateForm = {updateForm} 
        ...
        {formType === "complete"?<TodoComplete 
                                              name = {name}
                                              showModal = {showModal} 
...
export function TodoCreate ({showModal, 
                              createForm, 
                              setCreateForm,
  ...
  return (<div className={`form_modal`}>
        ...
        <div className='input-fields form'>
             ...
             <div className="dropdown">
                <button type="button" className="dropbtn">{createForm.category.length === 0 ? "(Hover to Select)" : createForm.category }</button>
                <div className="dropdown-content">
                  <button type="button" 
                          className="option-btn"
                          onClick={() => {handleOnCreateFormChange({target: { name: "category", value: "test"}})}}>test</button>
                  <button type="button" 
                          className="option-btn"
                          onClick={() => {handleOnCreateFormChange({target: { name: "category", value: "project"}})}}>project</button>
                  <button type="button" 
                          className="option-btn"
                          onClick={() => {handleOnCreateFormChange({target: { name: "category", value: "homework"}})}}>homework</button>
                  <button type="button" 
                          className="option-btn"
                          onClick={() => {handleOnCreateFormChange({target: { name: "category", value: "quiz"}})}}>quiz</button>
                  <button type="button" 
                          className="option-btn"
                          onClick={() => {handleOnCreateFormChange({target: { name: "category", value: "essay"}})}}>essay</button>
                </div>
              </div>
...
export function TodoUpdate ({ updateForm, 
                              setUpdateForm, 
                              handleOnUpdateFormChange, 
                              handleOnUpdateSubmit,
                              originalForm, 
    ...
    <div className='input-fields form'>
         ...
         <div className="dropdown">
                <button type="button" className="dropbtn">{updateForm.category.length === 0 ? "(Hover to Select)" : updateForm.category }</button>
                <div className="dropdown-content">
                  <button type="button" 
                          className="option-btn"
                          onClick={() => {handleOnUpdateFormChange({target: { name: "category", value: "test"}}, updateForm, setUpdateForm)}}>test/exam</button>
                  <button type="button" 
                          className="option-btn"
                          onClick={() => {handleOnUpdateFormChange({target: { name: "category", value: "project"}}, updateForm, setUpdateForm)}}>project</button>
                  <button type="button" 
                          className="option-btn"
                          onClick={() => {handleOnUpdateFormChange({target: { name: "category", value: "homework"}}, updateForm, setUpdateForm)}}>homework</button>
                  <button type="button" 
                          className="option-btn"
                          onClick={() => {handleOnUpdateFormChange({target: { name: "category", value: "quiz"}}, updateForm, setUpdateForm)}}>quiz</button>
                  <button type="button" 
                          className="option-btn"
                          onClick={() => {handleOnUpdateFormChange({target: { name: "category", value: "essay"}}, updateForm, setUpdateForm)}}>essay</button>
                </div>
              </div>
         </div>
         <div className='split-input-field form'>
         ...
         <div className='update-footer-buttons'>
            <button className = "form-submit-btn" type='button' onClick = {(event) => {handleOnUpdateSubmit(event, updateForm, setUpdateOrComplete)}}>Update</button>
...
export function TodoComplete ({setUpdateOrComplete, 
                              setCompleteForm, 
                              completeForm, 
                              handleOnCompleteFormChange,
                              handleOnCompleteSubmit, 
    ...
    <div className='input-fields form'>
         ...
         <div className='split-input-field form'>
            ...
            <input className='form-input split' name='peopleWith' type = "number" placeholder='People worked with' value={completeForm.peopleWith} onChange = {(event) => {handleOnCompleteFormChange(event, completeForm, setCompleteForm)}}/>
            </div>
            <div className='input-field form-split'>
            <span className='task-name'>Time Spent</span>
            <input className='form-input split' type = "number" name='timeSpent' placeholder='Time in minutes' onChange = {(event) => {handleOnCompleteFormChange(event, completeForm, setCompleteForm)}}/>
            </div>
          </div>
          <div className='input-field form'>
         <span className='task-name'>Comments</span>
         <textarea className='form-input description' name='comment' type = "text"placeholder='Add any comments' onChange = {(event) => {handleOnCompleteFormChange(event, completeForm, setCompleteForm)}}></textarea>
          
         </div>

         <div className='text-and-checkbox-area'>
          <p>Completed On Time?</p>
          <input className='checkbox-right' type = "checkbox" name='onTime' onChange = {(event) => {handleOnCompleteFormChange(event, completeForm, setCompleteForm)}}/>
         </div>
         <div className='text-and-checkbox-area'>
          <p>Public?</p>
          <input className='checkbox-right' type = "checkbox" name='public' onChange = {(event) => {handleOnCompleteFormChange(event, completeForm, setCompleteForm)}}/>
         </div>
         <div>
            <p className='error-create'>{taskError ? taskError : ""}</p>
          </div>
